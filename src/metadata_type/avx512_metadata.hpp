...
{ 
  ...
  explicit avx512_metadata() noexcept
    :group_{_mm512_loadu_si512(reinterpret_cast<const __m512i*>(EmptyMetadataGroup()))}
  {
  }
  ...
  [[nodiscard]] bit_mask Match(const metadata md) const noexcept {
    ...
    return bit_mask(static_cast<uint64_t>(_cvtmask64_u64(_mm512_cmpeq_epi8_mask(match, group_))));
  }
  ...
  [[nodiscard]] bit_mask getFirstBits() const noexcept {
    return bit_mask(static_cast<uint64_t>(_cvtmask64_u64(_mm512_movepi8_mask(group_))));
  }
